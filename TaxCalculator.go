...
func calcPhilHealth (monthlyIncome float64, ch chan float64) {
	ch <- math.Min(90000, math.Max(monthlyIncome, 10000)) * 0.04 / 2 
...
func calcSSS (monthlyIncome float64, ch chan float64) {
	IncomeRange := 0
	switch {
	case monthlyIncome < 4250: IncomeRange = 8
	case monthlyIncome > 29750: IncomeRange = 60
	default: IncomeRange = int ((monthlyIncome + 250) / 500)
	}
...
func calcIncomeTax (TaxableIncome float64) float64 {
	var result float64
	switch {
	case TaxableIncome <=  20833 : result =      0.00 
	case TaxableIncome <=  33332 : result =   	 0.00 + .2  * (TaxableIncome - 20833)
	case TaxableIncome <=  66666 : result =   2500.00 + .25 * (TaxableIncome - 33333)
	case TaxableIncome <= 166666 : result =  10833.33 + .3  * (TaxableIncome - 66667)
	case TaxableIncome <= 666666 : result =  40833.33 + .32 * (TaxableIncome - 166667)
	default :					   result = 200833.33 + .35 * (TaxableIncome - 666667)
	}
